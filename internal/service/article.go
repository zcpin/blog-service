...
type CreateArticleRequest struct {
	Title         string `form:"title" binding:"required,min=4,max=100"`
	...
	State         uint8  `form:"state,default=1" binding:"required,oneof=0 1"`
	CreatedBy     string `form:"created_by" binding:"required,min=2"`
	TagID         uint32 `form:"tag_id" binding:"required,gte=1"`
}
type UpdateArticleRequest struct {
	ID            uint32 `form:"id" binding:"required,gte=1"`
	Title         string `form:"title" binding:"min=4,max=100"`
	Desc          string `form:"desc" binding:"min=10,max=255"`
	Content       string `form:"content" binding:"min=10"`
	CoverImageUrl string `form:"cover_image_url" binding:"max=255"`
	State         uint8  `form:"state,default=1" binding:"oneof=0 1"`
	ModifiedBy    string `form:"modified_by" binding:"required,min=2"`
	TagID         uint32 `form:"tag_id" binding:"required,gte=1"`
}
...
func (svc *Service) GetArticle(param *ArticleRequest) (*Article, error) {
	...
	tag, err := svc.dao.GetTag(articleTag.TagId, 1)
	if err != nil {
		return nil, err
	}
	return &Article{
